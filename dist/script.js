...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_3__["default"])(product);
  Object(_modules_checkMask__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="phone"]');
...
function replacement(blockSelector) {
  var block = document.querySelectorAll(blockSelector);

  function showImg(blockImg) {
    var img = blockImg.querySelector('img');
    img.src = img.src.slice(0, -4) + '-1.png';
    blockImg.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
...
function calc(promocode, blockSize, blockMaterial, blockOptions, price, button, product) {
  var promocod = document.querySelectorAll(promocode),
  ...
  function createProduct(elem, type) {
    // добавление наименования услуги
    elem.forEach(function (item) {
...
function getCards(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResourse"])('http://localhost:3000/styles').then(function (item) {
      addCards(item);
    }).catch();
    btn.remove();
  });
...
/* harmony default export */ __webpack_exports__["default"] = (checkMask);
...
function drop() {
  ...
  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(function (fileName) {
  ...
  ['dragenter', 'dragover'].forEach(function (fileName) {
    fileInputs.forEach(function (input) {
      input.addEventListener(fileName, function () {
        return highLight(input);
      }, false);
    });
  });
  ['dragleave', 'drop'].forEach(function (fileName) {
    fileInputs.forEach(function (input) {
      input.addEventListener(fileName, function () {
